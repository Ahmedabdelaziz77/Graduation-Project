...
function FilterSection({ categories }) {
  ...
  // ✅ Sync local state if user updates URL manually
  useEffect(() => {
    const urlCategory = searchParams.get("category") || "all";
    const urlDiscount = Number(searchParams.get("discount")) || 0;
    const urlPrice = searchParams.get("price")?.split(",").map(Number) || [
      0, 30000,
    ];

    if (urlCategory !== selectedCategory) setSelectedCategory(urlCategory);
    if (urlDiscount !== selectedDiscount) setSelectedDiscount(urlDiscount);
    if (
      urlPrice[0] !== selectedPriceRange[0] ||
      urlPrice[1] !== selectedPriceRange[1]
    ) {
      setSelectedPriceRange(urlPrice);
    }
  }, [searchParams, selectedCategory, selectedDiscount, selectedPriceRange]);
